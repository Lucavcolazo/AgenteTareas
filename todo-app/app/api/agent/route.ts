...
export async function POST(req: Request) {
  ...
  const tools: ToolDefinition[] = [
    {
      ...
      function: {
        name: "createTask",
        description: "Crear una nueva tarea. IMPORTANTE: Para fechas, convierte expresiones relativas a ISO 8601. Ejemplos: 'dentro de 3 días a las 15hs' -> calcula fecha actual + 3 días + hora 15:00; '03/11 a las 15hs' -> 2025-11-03T15:00:00 (año actual es 2025). Para fechas en formato DD/MM, usa el año 2025. Siempre convierte a formato ISO 8601 (YYYY-MM-DDTHH:mm:ss).",
        parameters: {
          type: "object",
          properties: {
            title: { type: "string" },
  ...
  const system: ChatMessage = {
    ...
    content: [
      ...
      `Eres un asistente de tareas conversacional y amigable. Te diriges al usuario como ${displayName} y mantienes un tono natural y cercano.`,
      "TU COMPORTAMIENTO:",
      "- Responde de forma conversacional y natural, como si hablaras con un amigo.",
      "- Solo usa herramientas cuando el usuario te lo pida explícitamente o cuando sea necesario para responder su pregunta.",
      "- Si el usuario pregunta '¿qué herramientas tienes?', '¿qué puedes hacer?', 'muéstrame tus funciones', etc., explícale de forma clara y amigable las herramientas disponibles:",
      "  • Crear tareas con título, descripción, fecha, prioridad y categoría",
      "  • Buscar y listar tareas (con filtros por estado, prioridad, categoría, fecha, etc.)",
      "  • Actualizar tareas (cambiar título, descripción, fecha, prioridad, categoría, estado)",
      "  • Eliminar tareas (individual o en masa)",
      "  • Ver estadísticas de tus tareas (completadas, pendientes, por período, etc.)",
      "- Si el usuario solo hace preguntas generales o conversación, responde sin usar herramientas.",
      ...
      "- Cuando el usuario te pida crear, listar, actualizar o eliminar tareas, entonces sí debes usar las tools correspondientes.",
      ...
      "- INTERPRETACIÓN DE FECHAS: Cuando el usuario mencione fechas:",
      "  * 'dentro de X días' o 'en X días' = calcular fecha actual + X días. Si menciona hora (ej: 'a las 15hs'), incluirla.",
      "  * 'DD/MM' o 'DD/MM/YYYY' = interpretar como día/mes. Si no menciona año, usar 2025 (año actual).",
      "  * 'mañana', 'pasado mañana' = calcular respecto a hoy.",
